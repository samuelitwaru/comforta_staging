class DataMapper {
    ...
    getTiles() {
        ...
        this.rows.forEach(row => {
          ...
          row.components.forEach(tile => {
            console.log(this.getTileText(tile))
    ...
    getTileData(tile) {
        let tileData = {}
        tileData.text = this.getTileText(tile)
        tileData.textColor = this.getTileTextColor(tile)
        tileData.textAlignment = this.getTileTextAlignment(tile)
        tileData.icon = this.getTileIcon(tile)
        tileData.iconColor = this.getTileIconColor(tile)
        tileData.iconAlignment = this.getTileIconAlignment(tile)
        tileData.bgImage = this.getTileBGImage(tile)
        tileData.bgColor = this.getTileBGColor(tile)
        return tileData
    }

    getTileText(tile) {
        let tileText = tile.components[0].components.find(comp => comp.classes.includes('tile-title-section')).components.find(comp=>comp.classes.includes('tile-title')).components[0].content
        return tileText
    }
    ...
    getTileBGColor(tile) {
        let id = tile.components[0].components.find(comp => comp.classes.includes('template-block'))?.attributes?.id
    ...
    getTileTextAlignment(tile) {
        ...
        return this.getStyle(id, 'textAlign')
    }
    ...
    getTileIconColor(tile) {
        let svg = tile.components[0].components.find(comp => comp.classes.includes('tile-icon-section'))
        .components.find(comp=>comp.classes.includes('tile-icon')) //
        .components[0] //
        .components[0] //
        console.log(svg)
        let iconColor = svg.attributes.fill
        return iconColor 
    }
    ...
    getTileIconAlignment(tile) {
      ...
      let val = this.getStyle(id, 'alignSelf')
      if (val == 'start') {
        return 'left'
      } else if (val == 'end') {
        return 'right'
