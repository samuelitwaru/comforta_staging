class EditorManager {
    ...
    shouldShowAppBar(page) {
      return page.PageIsContentPage || (page.PageIsPredefined && page.PageName !== "Home");
    }
  ...
  class ToolBoxManager {
      ...
      imageOpacity.addEventListener("input", (event) => {
        ...
        if (this.editorManager.selectedTemplateWrapper) {
          ...
          if (templateBlock) {
            ...
            const currentBgImage = templateBlock.getStyle()['background-image'].match(/url\((.*?)\)/)[1];
            templateBlock.addStyle({
                "background-image": `linear-gradient(rgba(255, 255, 255, ${1 - value/100}), rgba(255, 255, 255, ${1 - value/100})), url(${currentBgImage})`
            });
          }
        }
      });
    ...
    activateCtaBtnStyles(selectedCtaComponent) {
      if (selectedCtaComponent) {
        document.querySelector(".cta-button-layout-container").style.display = "flex";
      } 
    }
