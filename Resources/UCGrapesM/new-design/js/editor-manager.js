  ...
  loadEditor(newEditorInstance, page, parentId) {
    ...
    newEditorInstance.on("component:selected", (component) => {
      ...
      if (this.selectedTemplateWrapper) {
        ...
        const button = component.getEl(); // Get the actual DOM element of the selected component
        console.log("Button is: ", button);

        if (!button || !button.classList.contains("action-button")) return;

        const templateWrapper = button.closest(".template-wrapper"); // Find the closest template-wrapper
        if (!templateWrapper) {
          console.error("Template wrapper not found!");
          return;
        }

        console.log("Template Wrapper is: ", templateWrapper.id);

        this.templateComponent = newEditorInstance.Components.getById(
          templateWrapper.id
        );

        if (!this.templateComponent) {
          console.error("Template component not found!");
          return;
        }

        console.log(
          "Template Component ID is: ",
          this.templateComponent.getId()
        );

        if (button.classList.contains("delete-button")) {
          this.deleteTemplate(this.templateComponent);
        } else if (button.classList.contains("add-button-bottom")) {
          this.addTemplateBottom(this.templateComponent, newEditorInstance);
        } else if (button.classList.contains("add-button-right")) {
          this.addTemplateRight(this.templateComponent, newEditorInstance);
        }
      }
        ...
        .then((parsedData) => {
          ...
          if (page.PageIsContentPage) {
            this.dataManager.getContentPageData(page.PageId).then((res) => {
              ...
              let img = this.editor.getWrapper().find("#product-service-image");
              let p = this.editor
                .getWrapper()
