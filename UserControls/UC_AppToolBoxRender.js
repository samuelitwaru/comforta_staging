function UC_AppToolBox($) {
	...
	var _iOnOnSave = 0; 
	var _iOnNewServiceButtonClicked = 0; 
	var $container;
	this.show = function() {
			...
			$(this.getContainerControl())
				.find("[data-event='NewServiceButtonClicked']")
				.on('newservicebuttonclicked', this.onNewServiceButtonClickedHandler.closure(this))
				.each(function (i) {
					this.setAttribute("data-items-index", i + 1);
				}); 
	...
	this.Scripts = [];
		...
		this.onNewServiceButtonClickedHandler = function (e) {
			if (e) {
				var target = e.currentTarget;
				e.preventDefault();
			...
			if (this.NewServiceButtonClicked) {
				this.NewServiceButtonClicked();
			}
		} 
